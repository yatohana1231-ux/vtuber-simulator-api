...
| `debugCharacterState.test.ts` | `src/handlers/debugCharacterState.ts` | `characterId` 未指定・不明な `packageId` で 400、`mood`・`perception` の検証（オブジェクトでない・配列・`null`・項目の不足・余分・文字列・小数・0・101 で 400。丸めない）、省略時は `undefined` のまま `run*` に渡ること（片方だけ・両方省略）、`character` に既定パッケージのキャラクターが渡ること、成功時 body は `{ mood, perception }`、例外時は 500（D-038） |
