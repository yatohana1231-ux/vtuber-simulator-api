...
6本のハンドラー（`absenceSimulator.ts` はまだエンドポイントとして公開していない）はいずれも「`parseRequestBody` → 入力チェック → `loadRequestedPackage` → `run*` 用の引数へ詰め替え → `run*` 呼び出し → `createResponse`」という同じ形をしているため、確認する観点も共通にしている。
...
| テストファイル | 対象 | 内容 |
|---|---|---|
| `absenceSimulator.test.ts` | `src/handlers/absenceSimulator.ts` | `eventResolver.test.ts` と同じ観点に加えて、`now` が `lastLoginAt` より前なら 400（同時刻は 200）、`lastLoginAt` だけ指定して `now` 省略時に `lastLoginAt` が未来なら 400、`run*` に既定パッケージの `lifestyle` と `timezone` を持つ `world` が渡ること |
