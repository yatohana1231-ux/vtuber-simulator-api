...
/** 行動の枠を展開する対象期間（今回ログインまでの直近）の上限時間。現行の actionPlanner と同じ */
 ...
 *   （now が lastLoginAt より前でも例外にしない。入力値の検証はフェーズ4のハンドラーで行う）。
 * - 行動の枠は、今回ログイン（now）までの直近 MAX_ACTION_WINDOW_HOURS 時間が上限
 *   （現行の actionPlanner と同じ）。
 * - 出来事の件数は、行動の枠の上限（12時間）では切らず、不在期間全体（elapsedMs）で決める。
 * - lastLoginAt / now が不正な Date（NaN）の場合の扱いは決めない（呼び出し元、
 *   フェーズ4のハンドラーで入力を検証する想定）。
 */
