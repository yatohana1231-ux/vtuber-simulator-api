...
**実装途中。** 2026-09-19 時点で、本体（`index.ts`）とハンドラー（[`../handlers/absenceSimulator.ts`](../handlers/README.md)）まであるが、ビルド対象・CDK のエンドポイントにはまだ入っていない（ロードマップのフェーズ5で旧 `eventResolver`/`actionPlanner` と差し替える）。記録の保存・読み出しは [`../lib/dynamo.ts`](../lib/README.md) にある。
...
| `prompts/absenceSimulator.fixed.mustache` / `prompts/absenceSimulator.variable.mustache` | 固定部・可変部のテンプレート。固定部の「書き方のルール」は出来事・行動・続いている話題・全体に分けて書き、可変部の末尾でルールと出力形式を守るよう念を押す（2026-09-19 に stg のモデルで試した結果をもとに調整。ロードマップの「状態」）。特定の世界観・キャラクターに依存する文面は書かない。差し込みはすべて `{{{ }}}`（HTML エスケープしない） |
